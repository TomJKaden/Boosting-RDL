"""
Adapted from RelGT under MIT License
Original source: https://github.com/snap-stanford/relgt/blob/main/utils.py
License: https://opensource.org/license/mit

Modifications have been made to fix errors and are marked.
"""
...
def init_worker_globals(adj, all_nodes, data):
    global GLOBAL_ADJ, GLOBAL_ALL_NODES, GLOBAL_DATA # Modified
    GLOBAL_ADJ = adj
    GLOBAL_ALL_NODES = all_nodes
    GLOBAL_DATA = data # Added
...
def local_nodes_hetero(
    ...
    """
    global GLOBAL_ADJ, GLOBAL_ALL_NODES, GLOBAL_DATA
    ...
    assert len(seed_node_idxs) == len(table_input_time), (
        "Mismatch in seed_node_idxs vs table_input_time"
    )
    ...
    with Pool(
        ...
        initargs=(adjacency, all_nodes_all_types, data),  # pass both adjacency and fallback
    ) as pool:
...
class RelGTTokens(Dataset):
    def __init__(
        ...
        self.target = (
            self.table_input.target if self.table_input.target is not None else None
        )
    ...
    def get_global_index(
        self, type_idxs: List[int], local_idxs: List[int]
    ) -> List[int]:
        """
            ...
            with tqdm(total=total, desc=f"Precomputing '{self.split}'") as pbar:
                for start_idx in range(0, total, chunk_size):
                    ...
                    for i, node_id in enumerate(chunk_node_idxs):
                        ...
                        for j, (t_str, nbr_loc_idx, hop, t_val, c1hops) in enumerate(
                            final_nodes
                        ):
